...
type MapMarkerProps = {
  ...
  /** Reference key for React list rendering */
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 pointer-events-none rounded-xl border border-slate-200 dark:border-slate-700 px-2 py-1 text-xs text-balance shadow-xl",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
...
function ControlGroup({ children }: { children: ReactNode }) {
  return (
    <div className="border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md [&>button:not(:last-child)]:border-slate-100 dark:[&>button:not(:last-child)]:border-slate-700 flex flex-col overflow-hidden rounded-xl border shadow-xl [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800",
